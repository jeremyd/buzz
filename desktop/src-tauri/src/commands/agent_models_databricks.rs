//! Databricks v1/v2 model discovery.
...
pub(super) fn databricks_sign_in_required_error() -> String {
    "Databricks authentication is required; set DATABRICKS_TOKEN in agent settings (interactive sign-in is not supported)"
        .to_string()
}
...
pub(super) async fn discover_databricks_models(
    ...
    .await
    {
        ...
        {
            // The interactive browser sign-in was removed with the external
            // login flows — a workspace token in the environment is the
            // supported path.
