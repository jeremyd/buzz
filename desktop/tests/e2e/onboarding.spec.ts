...
test("create a community offers the self-hosted connect form", async ({
  page,
}) => {
  await seedActiveIdentity(page, BLANK_TYLER_IDENTITY);
  await page.addInitScript((pubkey) => {
  ...
  await expect(page.getByText(/a relay you run yourself/)).toBeVisible();
  await expect(page.getByPlaceholder("Your community relay URL")).toBeVisible();
});
